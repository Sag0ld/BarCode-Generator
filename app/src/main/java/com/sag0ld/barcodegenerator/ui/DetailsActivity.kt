...
class DetailsActivity : AppCompatActivity(), ShowFragment.OnFragmentInteractionListener,
    ...
    override fun onResume() {
        ...
        model.bitmapLiveData.observe(this, Observer<Bitmap> {
            it?.let { bitmap ->
                codeImageViewDetails.setImageBitmap(bitmap)
            }
        })
    }
    ...
    override fun updateBarcode(content: String, type: String) {
        model.barcodeLiveData.value?.let { barcode ->
            if (barcode.content != content || barcode.type != type) {
                barcode.content = content
                barcode.type = type
